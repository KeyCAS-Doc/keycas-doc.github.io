...
# 基础模块
## 图片(image)、文本(text) 与容器 (container)
在本编辑器中，图片、文本和容器是三个最基本的元素。您可以将它们排列编辑来创造视觉效果，但学习如何排版并不是一件容易事。所幸，我们为您准备了现成的排版模块——"card container"。移至下一步来进行实践。

（此处应有gif）

{: .note}
>恭喜您解锁了网站编辑的核心功能——排版👏\
>除了文本和图片，容器还可以容纳按钮、卡片等其他元素\
>当然，如果您认为新建一个容器过于麻烦，您也可以选择直接使用“card container”来快速、高效地创造视觉效果。

{: .important}
>如果您在制作网站的过程中发现选中容器十分困难，您可以在左侧工具栏<img src="../images/layout.png" alt="lay out" width="20"/>中选中容器。
>这个功能在之后的网页编辑中也将派上用场。您还可以在其中整理、归纳您网页中的分层


### 卡片容器（card）
在左侧工具栏中找到“card container”，并将其拖拽至画布上。卡片是一个十分实用的，容纳图片+文本的组合模块，你可以通过它图文并茂地地展示你的内容。在一个卡片中，双击灰色区域选择图片插入，双击文本编辑内容。
...
## 输入 (Input)
目前输入功能无法使用，可以无视。同时，含有输入功能的Form也无法使用
